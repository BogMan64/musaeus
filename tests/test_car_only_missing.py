...
class TestItAsksTheCatalogueWhatIsMissing:
    def test_rows_with_a_car_file_are_excluded(self, conn, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(B, "get_config", lambda: type(
            "C", (), {"alac_library": tmp_path, "alac_archive": tmp_path})())

        staging = tmp_path / "stage"
        staging.mkdir()
        files, tracks, _, _ = B.stage_from_catalogue(
            conn, staging, only_missing=True)
        seen["n"] = len(files)
    ...
    def test_without_the_flag_everything_is_staged(self, conn, tmp_path, monkeypatch):
        ...
        import musaeus.editions as E
        monkeypatch.setattr(E, "select_edition",
                            lambda c, s, budget_bytes=None: _Sel([_Track(done), _Track(gap)]))
        monkeypatch.setattr(E, "master_path_for",
                            lambda fp, lib, arc: __import__(
                                "musaeus.editions", fromlist=["MasterResolution"]
                            ).MasterResolution(Path(fp), True))
        monkeypatch.setattr(E, "output_path_for",
                            lambda t, s, st: st / Path(t.file_path).name)
        monkeypatch.setattr(B, "get_config", lambda: type(
            "C", (), {"alac_library": tmp_path, "alac_archive": tmp_path})())
