...
def _db():
    ...
    c.execute(
        "CREATE TABLE archive (file_path TEXT, artist TEXT, album TEXT, "
...
def test_genre_validate_flags_non_canonical_spelling():
    ...
    c = _db()
    c.execute(
        "INSERT INTO archive VALUES ('/a.m4a','Gwen Stefani','A','Hollaback Girl','pop','CATALOGUED')"
    )
    st = GenreValidateStage()
...
def test_genre_validate_accepts_slash_variant():
    ...
    c = _db()
    c.execute(
        "INSERT INTO archive VALUES ('/a.m4a','James Brown','A','T','R&B/Funk/Soul','CATALOGUED')"
    )
    st = GenreValidateStage()
