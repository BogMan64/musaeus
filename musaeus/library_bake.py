...
from musaeus.deep_scan import ensure_columns as _deep_scan_ensure_columns
from musaeus.handoff import write_tool_handoff
from musaeus.idle_throttle import IdleThrottle
from musaeus.sleep_inhibit import reexec_under_inhibitor
from musaeus.stages.corrupt import ffmpeg_decode_check
...
def _archive_dir(cli_value: str | None, vault_root: Path) -> Path:
    ...
    return vault_root / "Libraries" / "ALAC-Archival"
...
def ffmpeg_measure_loudnorm(
    ...
    result = (
        _run_with_deadline(cmd, timeout)
        if timeout
...
def _decode_gate(conn, row_id: int, source: Path, execute: bool) -> str:
    ...
    if checked["decode_checked_at"] and checked["decode_ok"] == 0:
        return (
            f"SKIP  {source.name}: master fails to decode -- not baked "
